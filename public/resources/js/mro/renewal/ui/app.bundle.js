      ...
      dp.bind('open', function () {
        ...
        setTimeout(function () {
          ...
          if (spaceBelow < calendarH && spaceAbove > spaceBelow) {
            ...
            $animContainer[0].style.cssText = 'display: block; position: absolute; top: ' + newTop + 'px !important; left: ' + newLeft + 'px; z-index: 10014;';
          }
...
(function ($, window) {
  ...
  function bindMoreToggle() {
    $(document).off('click' + EVENT_NS, MORE_BTN).on('click' + EVENT_NS, MORE_BTN, function () {
      var $btn = $(this);
...
(function ($, window, document) {
  ...
  function bind() {
    ...
    // 스크롤 감지 — portal이 k-window 내부일 때만 닫기 (capture phase)
...
(function ($, window) {
  ...
  function initLegendSwiper() {
    ...
    if (!containers.length || !swiper_bundle/* default */.A) {
      return;
    }
    containers.forEach(function (container) {
      if (!container) {
