...
(function ($) {
  ...
  // 기본 탭형
  function initTab() {
    $(SCOPE).not('[data-type]').each(function (idx, el) {
      var $root = $(el);
      if ($root.data('init')) return;
      ...
      var autoplayVal = getInt(el, 'autoplay');
      if (autoplayVal) contentConfig.autoplay = {
        delay: autoplayVal,
        disableOnInteraction: false
      };
      var contentSwiper = new swiper_bundle/* default */.A(contentEl, contentConfig);
  ...
  function initTabHscroll() {
    $(SCOPE).filter('[data-type="hscroll"]').each(function (idx, el) {
      var $root = $(el);
      if ($root.data('init')) return;
      var tabEl = $root.find('.tab-area > .swiper')[0];
      var contentEl = $root.find('.content-area > .swiper')[0];
